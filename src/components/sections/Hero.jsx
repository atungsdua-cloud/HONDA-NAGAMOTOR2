...
const specBadges = [
  { icon: FiZap, label: 'Proses Cepat', color: 'from-yellow-400 to-orange-500' },
  { icon: FiShield, label: 'Bergaransi', color: 'from-green-400 to-emerald-500' },
  { icon: FiClock, label: '0% APR', color: 'from-blue-400 to-cyan-500' },
]

function FloatingBadge({ icon: Icon, label, color, index }) {
  return (
    <motion.div
      initial={{ opacity: 0, scale: 0 }}
      animate={{ opacity: 1, scale: 1 }}
      transition={{ duration: 0.5, delay: 1.5 + index * 0.15, ease: 'easeOut' }}
      className="absolute hidden lg:flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/5 backdrop-blur-md border border-white/10 shadow-lg"
      style={{
        top: `${20 + index * 18}%`,
        right: `${-10 + index * 8}%`,
      }}
    >
      ...
      <span className="text-white/80 text-[10px] font-semibold tracking-wide">{label}</span>
    </motion.div>
  )
}
...
export default function Hero() {
  ...
  return (
    ...
    >
      ...
      >
        <div className="flex flex-col lg:flex-row items-center lg:items-start gap-10 lg:gap-16">
          <div className="flex-1 max-w-2xl pt-4 lg:pt-8">
            ...
            <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl xl:text-7xl font-poppins font-black text-white leading-[1.1] mb-2 tracking-tight">
          ...
          >
            <div className="relative">
              ...
              <div className="absolute -inset-[1px] bg-gradient-to-b from-white/20 via-transparent to-white/5 rounded-2xl" />
              <div className="relative bg-white/[0.06] backdrop-blur-xl rounded-2xl border border-white/10 overflow-hidden shadow-2xl">
                ...
                {/* Glow corner */}
                <div className="absolute -top-20 -right-20 w-40 h-40 bg-honda-red/10 rounded-full blur-3xl" />
                ...
                <div className="p-5 sm:p-8">
                  <div className="flex flex-col items-center text-center">
                    ...
                    <div className="relative mb-4 sm:mb-5">
                      ...
                      <div className="rounded-full p-[3px] bg-gradient-to-b from-white/20 to-white/5">
                        <div className="rounded-full p-0.5 bg-gray-950">
                          <img
                      ...
                      <div className="absolute -bottom-0.5 -right-0.5 w-6 h-6 sm:w-7 sm:h-7 bg-green-500 rounded-full border-[3px] border-gray-950 shadow-lg flex items-center justify-center">
                        <svg className="w-2.5 h-2.5 sm:w-3 sm:h-3 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" /></svg>
                      </div>
                    </div>

                    {/* Name & title */}
                    <h3 className="text-white font-semibold text-base sm:text-lg">{profile.name || 'Sales Honda'}</h3>
                    <p className="text-white/50 text-xs sm:text-sm mb-4 sm:mb-6">{profile.title || 'Sales Consultant'}</p>

                    {/* Rating bar */}
                    <div className="flex items-center gap-1 mb-4 sm:mb-6">
                      {[...Array(5)].map((_, i) => (
                        <svg key={i} className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-yellow-400 fill-yellow-400" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" /></svg>
                      ))}
                    </div>
      ...
      <div className="absolute top-1/4 right-0 z-20 hidden xl:block">
        {specBadges.map((badge, i) => (
          <FloatingBadge key={badge.label} {...badge} index={i} />
        ))}
      </div>

      {/* Slide indicators */}
      <div className="absolute bottom-24 md:bottom-8 left-1/2 -translate-x-1/2 z-10 flex items-center gap-3">
        {heroImages.map((_, i) => (
          <motion.button
            key={i}
